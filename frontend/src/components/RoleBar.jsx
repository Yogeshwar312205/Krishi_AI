...
export const RoleBar = () => {
  const { activeRole, setActiveRole, user, setAuth } = useAppStore();

  const roles = [
    {
      id: 'Farmer',
      label: 'Farmer Dashboard',
      sublabel: 'Price Forecast & Cold Chain',
      icon: Sprout,
    ...
    {
      id: 'Driver',
      label: 'Driver & Fleet Dashboard',
      sublabel: 'VRP Navigation & Trip Earnings',
      icon: Truck,
    ...
    {
      id: 'APMC Buyer',
      label: 'APMC Buyer Dashboard',
      sublabel: 'Procurement Board & Inbound Mandi',
      icon: Store,
  ...
  const handleRoleSelect = (roleId) => {
    ...
    // Sync user role state if logged in for smooth demoing
    if (user) {
      const demoNames = {
      ...
      setAuth({ ...user, role: roleId, name: demoNames[roleId] || user.name }, localStorage.getItem('token') || 'demo-token');
    }
  };
      ...
      <div className="flex items-center justify-between px-1">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-xs font-black text-slate-800 uppercase tracking-wider">
            Switch Dashboard Role Mode
          </span>
        </div>
        <span className="text-[11px] text-slate-500 font-semibold hidden sm:inline">
          Active View: <strong className="text-forest-900 font-extrabold">{activeRole}</strong>
        </span>
      </div>
